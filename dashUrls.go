...
func initAdminUrlPatterns(r *kmux.Router, StaticAndTemplatesEmbeded ...embed.FS) {
	...
	r.GET("/admin", Admin(IndexView))
	r.GET("/admin/login", Auth(LoginView))
	r.POST("/admin/login", Auth(LoginPOSTView))
	r.GET("/admin/logout", LogoutView)
	r.POST("/admin/delete/row", Admin(DeleteRowPost))
	r.POST("/admin/update/row", Admin(UpdateRowPost))
	r.POST("/admin/create/row", Admin(CreateModelView))
	r.POST("/admin/drop/table", Admin(DropTablePost))
	r.GET("/admin/table/model:str", Admin(AllModelsGet))
	r.POST("/admin/table/model:str/search", Admin(AllModelsSearch))
	r.GET("/admin/get/model:str/id:int", Admin(SingleModelGet))
	r.GET("/admin/export/table:str", Admin(ExportView))
	r.POST("/admin/import", Admin(ImportView))
	if Pprof {
